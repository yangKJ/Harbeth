    ...
    func setupContentView() -> some View {
        List {
            ...
            Group {
                ...
                NavigationLink(destination: CustomViews(value: CISketch.range.value, filtering: {
                    CISketch.init(radius: $0)
                }, min: CISketch.range.min, max: CISketch.range.max, inputImage: R.image("IMG_4931"))) {
                    Text("CI Sketch filter")
                }
            }
        }
        .padding(.bottom)
