...
kernel void C7Sobel(texture2d<half, access::write> outputTexture [[texture(0)]],
    ...
    const half4 h = -topLeft - 2.0 * top - topRight + bottomLeft + 2.0 * bottom + bottomRight;
    const half4 v = -bottom - 2.0 * centerLeft - topLeft + bottomRight + 2.0 * centerRight + topRight;
