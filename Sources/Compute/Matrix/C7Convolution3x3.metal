...
kernel void C7Convolution3x3(texture2d<half, access::write> outputTexture [[texture(0)]],
                             ...
                             constant float *m11 [[buffer(0)]],
                             constant float *m12 [[buffer(1)]],
                             constant float *m13 [[buffer(2)]],
                             constant float *m21 [[buffer(3)]],
                             constant float *m22 [[buffer(4)]],
                             constant float *m23 [[buffer(5)]],
                             constant float *m31 [[buffer(6)]],
                             constant float *m32 [[buffer(7)]],
                             constant float *m33 [[buffer(8)]],
    ...
    const float l = float(x - 1);
    const float r = float(x + 1);
    const float t = float(y - 1);
    const float b = float(y + 1);
    ...
    const float3x3 matrix = float3x3({*m11, *m12, *m13}, {*m21, *m22, *m23}, {*m31, *m32, *m33});
