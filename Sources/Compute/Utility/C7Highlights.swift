//
//  C7Highlights.swift
//  Harbeth
//
//  Created by Condy on 2026/2/10.
//

import Foundation

/// 高光调整滤镜
/// Highlights adjustment filter
public struct C7Highlights: C7FilterProtocol {
    
    /// Highlights adjustment, ranging from -1.0 to 1.0, with 0.0 being the original
    public static let range: ParameterRange<Float, Self> = .init(min: -1, max: 1, value: 0.0)
    
    @Clamping(range.min...range.max) public var highlight: Float = range.value
    
    public var modifier: ModifierEnum {
        return .compute(kernel: "C7Highlights")
    }
    
    public var factors: [Float] {
        return [highlight]
    }
    
    public init(highlight: Float = range.value) {
        self.highlight = highlight
    }
}
