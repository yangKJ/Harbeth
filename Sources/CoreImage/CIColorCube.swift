...
extension CIColorCube.Resource {
    static func readCubeResource(from url: URL) -> CIColorCube.Resource? {
        guard let contents = try? String(contentsOf: url),
              let dimension = CIColorCube.Resource.takeDimension(from: contents, pattern: CIColorCube.Resource.LUT_3D) else {
            return nil
        }
        let data = CIColorCube.Resource.cubeData(with: contents)
        return CIColorCube.Resource(dimension: dimension, data: data)
    }
    
    static func createLUTTexture(from resource: CIColorCube.Resource) -> MTLTexture? {
