...
public struct GuidedUpright: Sendable {
    ...
    private func sourceQuad(inputSize: CGSize, verticalGuides: [Guide], horizontalGuides: [Guide]) -> RenderQuadTransform.Quad? {
        ...
        let topLine: Line2D?
        let bottomLine: Line2D?
        let leftLine: Line2D?
        let rightLine: Line2D?
        ...
        case 1:
            ...
            if midpointY(of: candidate, size: inputSize) <= inputSize.height * 0.5 {
                ...
                bottomLine = Line2D(yConstant: bottomY)
            } else {
                topLine = Line2D(yConstant: topY)
        ...
        case 1:
            ...
            if midpointX(of: candidate, size: inputSize) <= inputSize.width * 0.5 {
                ...
                rightLine = Line2D(xConstant: rightX)
            } else {
                leftLine = Line2D(xConstant: leftX)
    ...
    private func line(for guide: Guide, size: CGSize) -> Line2D? {
        Line2D(
...
private struct Line2D {
    ...
    func intersection(with other: Line2D) -> CGPoint? {
