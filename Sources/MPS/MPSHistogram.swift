...
public struct MPSHistogram: MPSKernelProtocol {
    ...
    @Clamping(range.min...range.max) public var histogramEntries: Int = range.value {
        didSet {
            ...
            self.histogram = MPSImageHistogram(device: HarbethContext.shared.device, histogramInfo: &histogramInfo)
            self.equalization = MPSImageHistogramEqualization(device: HarbethContext.shared.device, histogramInfo: &histogramInfo)
        }
    }
    ...
    public func encode(commandBuffer: MTLCommandBuffer, textures: [MTLTexture]) throws -> MTLTexture {
        ...
        guard let histogramBuffer = HarbethContext.shared.device.makeBuffer(length: bufferLength, options: [.storageModePrivate]) else {
            return destTexture
        }
